...
function morsle_add_instance($data, $mform) {
    ...
    $collectionid = get_collection($title, $owner);

	// share resource with course user
	$base_feed = $morsle->docs_feed . $docid . '/acl';
    assign_file_permissions($owner, 'writer', $USER->email, $base_feed);

    // link resource to the read-only folder
	add_file_tocollection($morsle->docs_feed, $collectionid, $docid, $owner);
