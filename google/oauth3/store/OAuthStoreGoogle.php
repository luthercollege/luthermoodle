...
//$CFG->prefix
global $CFG;
require_once $CFG->dirroot.'/blocks/gmail/oauth3/store/OAuthStoreAbstract.class.php';
...
//                    );
        ...
        $secrets = array();
        if ( !$rec = get_record('gmail_oauth_consumer_token','user_id',$user_id) ) {
        ...
        $secrets['consumer_secret']   = get_config('blocks/gmail','oauthsecret');   //ocr_consumer_secret as consumer_secret, aka oauth secret
...
// The only Server we have is Google so don't bother checking
...
//        }
        ...
        if ( $rec = get_record('gmail_oauth_consumer_token', 'user_id', $user_id)  ) {
            $data->id = $rec->id;
            update_record('gmail_oauth_consumer_token', $data); 
        } else {
            insert_record('gmail_oauth_consumer_token', $data);
        }
